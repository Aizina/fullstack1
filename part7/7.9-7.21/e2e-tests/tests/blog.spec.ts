...
test.describe('Blog app', () => {
  ...
  test.describe('When logged in', () => {
    ...
    test('a new blog can be created', async ({ page }) => {
      ...
      await expect(
        page.getByText(
          'a new blog New blog from Playwright by Playwright Author added'
        )
      ).toBeVisible()
      await expect(
        page.locator('.blog', {
          hasText: 'New blog from Playwright Playwright Author',
        })
      ).toBeVisible()
    })
    ...
    test('a blog can be liked', async ({ page }) => {
      ...
      await page.getByRole('button', { name: 'Create' }).click()
      await expect(
        page.locator('.blog', { hasText: 'Blog to be liked Liker' })
      ).toBeVisible()
    ...
    test('the user who created a blog can delete it', async ({ page }) => {
      ...
      await page.getByRole('button', { name: 'Create' }).click()
      await expect(
        page.locator('.blog', { hasText: 'Blog to be deleted Deleter' })
      ).toBeVisible()

      const blog = page.locator('.blog', {
        hasText: 'Blog to be deleted Deleter',
      })
      await blog.getByRole('button', { name: 'view' }).click()
  ...
  test.describe('User permissions', () => {
    ...
    test('only the creator can see the delete button', async ({
      ...
      await page.evaluate(
        (u) => localStorage.setItem('loggedBlogAppUser', JSON.stringify(u)),
        mluukkaiUser
      )
      await page.reload()
      await expect(page.getByText('Matti Luukkainen logged in')).toBeVisible()
      ...
      await page.getByRole('button', { name: 'Create' }).click()
      await expect(
        page.locator('.blog', { hasText: 'Private Blog Secret Author' })
      ).toBeVisible()
      ...
      const blog = page.locator('.blog', {
        hasText: 'Private Blog Secret Author',
      })
      await blog.getByRole('button', { name: 'view' }).click()
