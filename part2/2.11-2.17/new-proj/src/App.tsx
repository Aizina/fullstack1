...
const App: React.FC = () => {
  ...
  useEffect(() => {
    DataService.getAll().then((initialPersons) => setPersons(initialPersons));
  }, []);

  const handleNameChange = (event: React.ChangeEvent<HTMLInputElement>) => setNewName(event.target.value);
  const handleNumberChange = (event: React.ChangeEvent<HTMLInputElement>) => setNewNumber(event.target.value);
    ...
    if (existingPerson) {
      if (window.confirm(`${existingPerson.name} is already added to the phonebook, replace the old number with a new one?`)) {
        ...
        DataService.update(existingPerson.id, updatedPerson)
          ...
          .catch((error) => {
            showMessage(`Error: The person "${existingPerson.name}" was already deleted from the server`, 'error');
